...
typedef void (*AfterReadCallback)(uint8_t the_register);
typedef void (*AfterWriteCallback)(uint8_t the_register, size_t num_bytes);
...
public:
    ...
    virtual void after_read(AfterReadCallback callback) = 0;
    virtual void after_write(AfterWriteCallback callback) = 0;// Add a callback to be notified when the master has read a register.
...
inline void after_read(AfterReadCallback callback) override {
    after_read_callback = callback;
}

inline void after_write(AfterWriteCallback callback) override {
    after_write_callback = callback;
}
...
*/
    AfterReadCallback after_read_callback = nullptr;
    AfterWriteCallback after_write_callback = nullptr;
