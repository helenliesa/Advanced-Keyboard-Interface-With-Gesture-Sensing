...
#include <stdint.h>
#include <stddef.h>
//#include <functional> not for atmega 


typedef void (*AfterReceiveCallback)(size_t length, uint16_t address);
typedef void (*TransmitCallback)(uint16_t address);
...
public:
    ...
    virtual void after_receive(AfterReceiveCallback callback) = 0; //typedefs used for compatibility
    virtual void before_transmit(TransmitCallback callback) = 0;
    virtual void after_transmit(TransmitCallback callback) = 0;
